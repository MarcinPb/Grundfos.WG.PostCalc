...
{
    [TestFixture]
    public class PumpBatteryBuilderTests
    {
        [Test]
        public void BuildPumpBattery_DumpDrawing()
        {
            var data = new PumpBattery
            {
                ID = 15003900,
            ...
            var builder = new PumpBatteryBuilder(colorServer);

            var result = builder.Build(data);
            document.Children.Add(result);
            var width = result.Bounds.Width;
            using (var file = new FileStream(@"C:\temp\test-pump-battery.svg", FileMode.Create))
            {
        ...
        [TestCase(123)]
        [TestCase(133)]
        public void BuildPumpBattery_SetSize_BoundariesAsExpected(double size)
        {
            var data = new PumpBattery
            {
                ID = 15003900,
        ...
        public void BuildPumpBattery_SetSize_CenterAsExpected(double x, double y)
        {
            var data = new PumpBattery
            {
                ID = 15003900,
