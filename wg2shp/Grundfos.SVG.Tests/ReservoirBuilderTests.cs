...
{
    [TestFixture]
    public class ReservoirBuilderTests
    {
        [Test]
        public void BuildReservoir_DumpDrawing()
        {
            var reservoirData = new Reservoir
            {
                ID = 15003900,
            ...
            var builder = new ReservoirBuilder(colorServer, closedPathBuilder, pathBuilder);

            var result = builder.Build(reservoirData);
            document.Children.Add(result);
            using (var file = new FileStream(@"C:\temp\test-reservoir.svg", FileMode.Create))
            {
        ...
        [TestCase(123)]
        [TestCase(133)]
        public void BuildReservoir_SetSize_BoundariesAsExpected(double size)
        {
            var reservoirData = new Reservoir
            {
                ID = 15003900,
        ...
        public void BuildReservoir_SetSize_CenterAsExpected(double x, double y)
        {
            var reservoirData = new Reservoir
            {
                ID = 15003900,
